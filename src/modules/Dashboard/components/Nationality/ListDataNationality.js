...
export default function ListDataNationality({ list = [], editData, deleteData, loading, column = 5, titles = [] }) {
  ...
  return (
    <div className="p-5">
      <div
        className={`grid justify-items-center grid-cols-table-6 auto-cols-min gap-5 p-6 bg-white w-full overflow-x-scroll lg:overflow-auto items-baseline max-w-lg sm:max-w-sm md:max-w-prose lg:max-w-none rounded-md `}
      >
        {titles.map((t, index) => (
        ...
        {list.map((l, index) => (
          <React.Fragment key={index}>
            ...
            <img src={l.flag_icon} alt="Flag" width={50} className="self-center" />
            ...
            <div className="flex gap-3 min-w-fit ">
              {checkAccess("Nationality", "nationality-edit") ? (
                <button
              ...
              {checkAccess("Nationality", "nationality-delete") ? (
                <button
