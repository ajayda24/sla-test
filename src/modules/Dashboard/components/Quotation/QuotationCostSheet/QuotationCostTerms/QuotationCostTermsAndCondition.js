...
export default function QuotationCostTermsAndCondition(props) {
  ...
  useEffect(() => {
    axios
      .get(`/dashboard/quotation-terms-and-condition-list/${quotationCostSheetId}`, {
        headers: {
  ...
  const submitForm = async (event) => {
    ...
    try {
      ...
      if (addOrEdit.method === "add") {
        response = await axios.post(`/dashboard/quotation-terms-and-condition`, body, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else if (addOrEdit.method === "edit") {
        console.log(addOrEdit);
        response = await axios.put(
          `/dashboard/quotation-terms-and-condition/${addOrEdit.editId}`,
      ...
      if (data) {
        navigate(`/dashboard/quotation/${quotationCostSheetId}/terms`);
        dispatch(
          setAppError({
  ...
  const editData = async (id) => {
    navigate(`/dashboard/quotation/${quotationCostSheetId}/terms#edit-${id}`);

    if (id) {
      setLoading((p) => ({ ...p, edit: true, loadingId: id }));
      try {
        const response = await axios.get(`/dashboard/quotation-terms-and-condition/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = response.data.data;
  ...
  const deleteData = async (id) => {
    ...
    if (id) {
      ...
      try {
        const response = await axios.delete(`/dashboard/quotation-terms-and-condition/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  return (
    <>
      ...
      <div className={`p-2  sm:p-5 `}>
        {!addOrEdit.open && (
          <>
            <Link to={`/dashboard/quotation/${quotationCostSheetId}/terms/create`}>
