...
export default function ListDataVendor({
  list = [],
  editData,
  deleteData,
  loading,
  ...
  return (
    <div className="p-5">
      <div
        className={`grid justify-items-center grid-cols-table-9 auto-cols-min gap-5 p-6 bg-white w-full overflow-x-scroll lg:overflow-auto items-baseline max-w-lg sm:max-w-sm md:max-w-prose lg:max-w-none rounded-md `}
      >
        {titles.map((t, index) => (
        ...
        {list.map((l, index) => (
          <React.Fragment key={index}>
            ...
            <hr
              className={`border-[1.5px] border-slate-100  w-full col-span-9 `}
            />
          </React.Fragment>
        ))}
      </div>
