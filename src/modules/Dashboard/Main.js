...
export default function Main({ navbarOpen, setNavbarOpen, blockData }) {
  ...
  return (
    <div
      className={`${
        navbarOpen && ""
      }  min-h-screen h-auto md:max-w-lg lg:max-w-xl xl:max-w-none min-w-full w-full  dark:bg-dark dark:text-white sm:pl-[17rem] ${
        dashboardPage === "Project" ? "bg-white" : "bg-slate-100"
      }`}
    >
        ...
        <div className="flex gap-6 items-center relative">
          ...
          <div className="peer">
            <img
              src={imageUrl}
              alt="user"
              className="w-7 h-7 rounded-full  mx-5"
            />
          </div>
          <div className="hidden absolute top-7 right-5 max-w-xs w-[17rem] h-auto bg-white shadow-md rounded-lg p-2 dark:bg-dark2 ring-1 ring-slate-400 dark:ring-black peer-hover:block hover:block">
            ...
            <p
              className="bg-white p-2 w-full hover:bg-red rounded-md hover:text-white dark:bg-dark2 dark:hover:bg-red flex items-center gap-3"
              onClick={() => {
      ...
      <div className="w-full overflow-x-scroll">
        {dashboardPage !== "Edit Profile" && <Outlet />}
      </div>
    </div>
  );
}
