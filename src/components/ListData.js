...
export default function ListData({ list = [], editData, deleteData, loading, column = 5, titles = [], tags = [] }) {
        ...
        {list.map((l, index) => (
          <React.Fragment key={index}>
            ...
            {dashboardPage === "Client" ? (
              ...
              <img src={getFlag(l.country_code)} alt="Flag" width={50} className="self-center" />
            ) : (
              dashboardPage === "Rate Category" && <p></p>
            )}
            <p>{dashboardPage === "Client" ? l.mobile : l.name}</p>

            <p>{dashboardPage === "Client" ? getFormattedDate(l.created_at) : l.country_code}</p>
            ...
            <hr className={`border-[1.5px] border-slate-100  w-full ${column === 5 ? "col-span-5" : "col-span-6"} `} />
          </React.Fragment>
        ))}
      </div>
    </div>
